...
import { getPages } from "../../../../../src/requests/User";
...
export default function Page() {
  ...
  const loadHaorDetail = async () => {
    setLoading(true);
    let response = await getPages(slug);
  ...
  return (
    <>
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{paddingBottom: 10}}>
          <View>
            <Image style={{justifyContent: 'center', height: 220, width: width, backgroundColor: '#ccc'}} source={{uri:web_url+page.header_image}}/>
            <View style={{ position: 'absolute', width: '100%', height: '100%', justifyContent: "center", alignItems: "center", backgroundColor: 'rgba(0,0,0,.3)' }}>
              <Text style={{color: '#FFFFFF', fontSize: 28, fontWeight: 500, marginBottom: 10}}>{page.title}</Text>
            </View>
          ...
          <HTML 
            ...
            source={{ html: page.content || '<p>&nbsp</p>' }} 
