...
export default function Page() {
  ...
  const filterHaorListByDistrict = (id) => {
    ...
    let hL = haorList.filter(u => String(u.d_id) === id)
  ...
  const filterHaorListByUpazila = (id) => {
    ...
    let hL = haorList.filter(u => String(u.u_id) === id)
    setHaorFilterList(hL)
    onChangeSearchHaorName('');
  };
  ...
  const filterHaorListByName = (name) => {
    ...
    if( searchUpazila && searchUpazila !== 'All' ){
      hL = haorList.filter(u => String(u.u_id) === searchUpazila && u.name.toLowerCase().includes(name.trim().toLowerCase()))
    }
    else if( searchDistrict && searchDistrict !== 'All' ){
      hL = haorList.filter(u => String(u.d_id) === searchDistrict && u.name.toLowerCase().includes(name.trim().toLowerCase()))
    }
    else if( name && name !== '' ){
          ...
          <View style={{width: width/2-12, borderWidth: 1, borderColor: '#cccccc'}}>
            ...
            <Picker
              ...
              {districtList && districtList?.map((x, index) => 
                  <Picker.Item label={x.name} value={String(x.id)} key={String(x.id)} />
                )
              }
            </Picker>
          </View>
